...
def train(conf: omegaconf.DictConfig) -> None:
    ...
    wandb_logger = WandbLogger(
        project=conf.dataset_name.split("/")[-1].replace(".py", ""), name=conf.model_name_or_path.split("/")[-1]
    )
    ...
    if conf.apply_early_stopping:
        callbacks_store.append(
            EarlyStopping(monitor=conf.monitor_var, mode=conf.monitor_var_mode, patience=conf.patience)
        )
